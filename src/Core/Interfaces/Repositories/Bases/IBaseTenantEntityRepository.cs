...
{
    public interface IBaseTenantEntityRepository<TEntity> where TEntity : BaseTenantEntity
    {
        Guid TenantId { get; }
        IBaseTenantEntityRepository<TEntity> SetTenantId(Guid tenantId);
        IQueryable<TEntity> GetDbSet(bool ignoreRemovedItens = true);
        IQueryable<TEntity> GetTenantDbSet(bool ignoreRemovedItens = true);
